...
import ktpack.manifest.*
import ktpack.util.*

/**
 * This class uses a Ktpack [ModuleToml] generated by a pack.toml manifest
 ...
 *
 * @param manifest The root manifest configuration to build.
...
class ModuleBuilder(
        ...
        val (newRoot, duration) = measureSeconds {
            val (mavenDependencies, mavenDuration) = measureSeconds {
                val deps = root.filter { it.dependencyConf is DependencyToml.Maven }
                    .resolveAndFlatten()
                context.resolver.resolveArtifacts(deps, releaseMode, target)
            }
    ...
    private suspend fun buildChildDependency(
        ...
        val childBuilder = ModuleBuilder(
    ...
    private suspend fun resolveMavenDependency(
        ...
        return context.resolver.resolve(node, false, targets.first())
    }
    ...
    private suspend fun resolveLocalDependency(
        ...
        val localModule = context.load(packFile.toString())
    ...
    private suspend fun startKotlinCompiler(
        ...
        val defaultJdk = checkNotNull(context.jdk.getDefaultJdk()) {
        ...
        val kotlincPath = when {
            target.isJs -> context.kotlinc.findKotlincJs(kotlinVersion)
            target.isNative -> context.kotlinc.findKotlincNative(kotlinVersion)
            else -> context.kotlinc.findKotlincJvm(kotlinVersion)
        }
    ...
    private fun MutableList<String>.configureJsCompilerArgs(
        ...
        val kotlincPath = Path(context.kotlinc.findKotlincJs(kotlinVersion))
