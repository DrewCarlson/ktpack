...
private fun parseToml(content: String): ManifestConf = memScoped {
    ...
    val license = toml_string_in(moduleTable, "license").toKStringFromUtf8()
    val repository = toml_string_in(moduleTable, "repository").toKStringFromUtf8()
    val kotlinVersion = toml_string_in(moduleTable, "kotlin-version").toKStringFromUtf8()
    ...
    val targetsArray = toml_array_in(moduleTable, "targets")
    val targets = if (targetsArray == null) {
        null
    } else {
        List(toml_array_nelem(targetsArray)) {
            val targetString = checkNotNull(toml_string_at(targetsArray, it).toKStringFromUtf8())
            Target.valueOf(targetString.uppercase())
        }
    }
